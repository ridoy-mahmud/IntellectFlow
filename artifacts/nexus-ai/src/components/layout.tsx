...
import {
  Rocket, LayoutDashboard, FolderKanban,
  Cpu, Network, BarChart3, Settings,
  Search, Bell, Menu, Plus, X, Activity,
  Clock, CheckCircle2, AlertTriangle
} from "lucide-react";
import { cn, GlassCard, GradientText } from "./ui-glass";
import { useListMissions, useGetRecentActivity } from "@workspace/api-client-react";
        ...
        <GlassCard className="overflow-hidden border-primary/20">
          <div className="flex items-center gap-3 p-4 border-b border-border">
            ...
            <div className="flex items-center gap-2">
              <kbd className="px-1.5 py-0.5 text-[10px] rounded bg-white/10 border border-white/20 text-muted-foreground">ESC</kbd>
          ...
          {query.trim().length < 2 ? (
            <div className="p-4">
              <p className="text-xs text-muted-foreground mb-3 font-semibold uppercase tracking-wider">Quick Navigation</p>
              <div className="space-y-1">
                {[
                  { label: "Mission Control", path: "/dashboard", icon: LayoutDashboard },
                  { label: "New Mission", path: "/dashboard/new-mission", icon: Plus },
                  { label: "Projects", path: "/dashboard/projects", icon: FolderKanban },
                  { label: "Analytics", path: "/dashboard/analytics", icon: BarChart3 },
                  { label: "Agent Studio", path: "/dashboard/agents", icon: Cpu },
                ].map(item => (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-muted-foreground text-sm">No missions found for "<span className="text-white">{query}</span>"</p>
            </div>
          ) : (
            <div className="p-2 space-y-1">
              <p className="text-xs text-muted-foreground px-3 pt-2 pb-1 font-semibold uppercase tracking-wider">Missions ({results.length})</p>
              {results.map(m => (
                <Link key={m.id} href={`/dashboard/mission/${m.id}`} onClick={onClose}>
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/5 cursor-pointer group">
                    <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                      ...
                      m.status === "failed" ? "bg-rose-400" : "bg-gray-400"
                    }`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white/90 group-hover:text-white truncate">{m.name}</p>
                      <p className="text-xs text-muted-foreground capitalize">{m.status} · {m.category || "General"}</p>
                    </div>
                    <span className="text-xs font-mono text-primary">{Math.round(m.progress || 0)}%</span>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="p-3 border-t border-border bg-white/2 flex items-center justify-between text-[10px] text-muted-foreground px-4">
            ...
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 rounded bg-white/10 border border-white/20">↵</kbd> select
              </span>
            </div>
          </div>
        </GlassCard>
        ...
        <div className="max-h-80 overflow-y-auto">
          {!recentActivity || recentActivity.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">Launch a mission to see agent notifications</p>
            </div>
          ) : (
            <div className="divide-y divide-border/30">
              {recentActivity.map((log, i) => (
                <div key={log.id} className="flex gap-3 p-3 hover:bg-white/3 transition-colors">
                  <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${
                    log.agentType === "system" ? "bg-white/30" : "bg-primary animate-pulse"
                  }`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`text-xs font-bold capitalize ${AGENT_COLORS[log.agentType] || "text-white/60"}`}>
                    ...
                    <p className="text-xs text-white/70 leading-relaxed line-clamp-2">{log.action}</p>
                  </div>
                </div>
              ))}
            </div>
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: recentActivity } = useGetRecentActivity({ limit: 5 }, { query: { refetchInterval: 15000 } });

  const hasNotifications = recentActivity && recentActivity.length > 0;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex overflow-hidden dark">
      ...
      >
        ...
        <div className="h-16 flex items-center px-4 border-b border-sidebar-border gap-3 overflow-hidden shrink-0">
          ...
          <AnimatePresence>
            {isSidebarOpen && (
              ...
              >
                <GradientText className="text-xl font-bold">IntellectFlow</GradientText>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        {/* User */}
        <div className={cn("p-3 border-t border-sidebar-border shrink-0", !isSidebarOpen && "flex justify-center")}>
            ...
            <AnimatePresence>
              {isSidebarOpen && (
                <motion.div
                  ...
                  className="overflow-hidden whitespace-nowrap"
                >
                  <p className="text-sm font-semibold text-white">Commander Jane</p>
        ...
        <header className="h-16 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-5 shrink-0 z-10">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground min-w-0">
              <Link href="/dashboard">
                <span className="hover:text-white transition-colors cursor-pointer shrink-0">Mission Control</span>
              </Link>
              {isSubPage && (
                <>
                  <span className="text-border shrink-0">/</span>
                  <span className="text-white font-medium truncate capitalize">{breadcrumb}</span>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="flex gap-1 shrink-0">
                <kbd className="px-1.5 py-0.5 text-[10px] rounded bg-white/10 border border-white/20">⌘</kbd>
                <kbd className="px-1.5 py-0.5 text-[10px] rounded bg-white/10 border border-white/20">K</kbd>
              </div>
            </button>
