        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              All missions and their current status.
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-24 bg-white/4 rounded-2xl animate-pulse border border-white/5"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                      <div className="flex-1 space-y-2 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                          <span className="flex items-center gap-1">
                            ...
                            {formatElapsed(mission.elapsedSeconds || 0)}
                          </span>
                          <span className="flex items-center gap-1">
                            <Cpu className="w-3.5 h-3.5" />{" "}
                            {(mission.tokensUsed || 0).toLocaleString()} tokens
                          </span>
                        ...
                        {(mission.status === "completed" ||
                          ...
                          <div className="flex items-center gap-2 text-[11px] mt-2 text-emerald-300">
                            <span className="px-2 py-0.5 rounded-full border border-emerald-500/30 bg-emerald-500/10">
                              results ready
                            </span>
                            <span className="text-muted-foreground">
                              video + text + research bundle
                            </span>
                            <Link
                              href={`/dashboard/mission/${mission.id}?tab=results`}
                            >
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        <div className="flex gap-1 flex-wrap">
                          {(Array.isArray(mission.agents) ? mission.agents : [])
