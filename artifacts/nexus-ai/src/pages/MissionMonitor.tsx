...
import {
  Play, Pause, Square, Download, Activity, Cpu, Code2, MessageSquare,
  Terminal, ArrowLeft, Loader2, AlertTriangle, CheckCircle2, RefreshCw,
  Search, PenLine, BarChart2, Palette, TestTube2, Clock
} from "lucide-react";
import { DashboardLayout } from "@/components/layout";
import { GlassCard, StatusBadge, CyberButton, AgentBadge } from "@/components/ui-glass";
import {
  useGetMission, useGetMissionActivity,
  usePauseMission, useResumeMission, useAbortMission
...
export default function MissionMonitor() {
  ...
  const { data: mission, isLoading, error } = useGetMission(missionId, {
    query: { refetchInterval: (query) => {
      const data = query.state.data as any;
      if (data?.status === "running" || data?.status === "paused") return 4000;
      return false;
    }}
  });

  const { data: activityLogs, refetch: refetchLogs } = useGetMissionActivity(missionId, { limit: 100 }, {
    query: { refetchInterval: (query) => {
      return 5000;
    }}
  });
  ...
  const invalidateAll = () => {
    ...
    queryClient.invalidateQueries({ queryKey: [`/api/missions/${missionId}/activity`] });
  ...
  const handleAbort = () => {
    if (!confirm("Are you sure you want to abort this mission? This cannot be undone.")) return;
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-[80vh] gap-4 text-center">
          ...
          <p className="text-muted-foreground max-w-sm">We couldn't find mission NX-{missionId}. It may have been removed.</p>
          <CyberButton variant="outline" onClick={() => setLocation("/dashboard")}>
  ...
  const isMutating = pauseMut.isPending || resumeMut.isPending || abortMut.isPending;
        ...
        <div className="shrink-0 p-5 border-b border-border bg-background/50 backdrop-blur-md z-10">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                  ...
                  {isMutating && <Loader2 className="w-4 h-4 text-primary animate-spin" />}
                </div>
                <div className="flex items-center gap-5 text-sm text-muted-foreground flex-wrap">
                  ...
                  <span className="flex items-center gap-1.5">
                    <Cpu className="w-3.5 h-3.5" /> {(m.tokensUsed || 0).toLocaleString()} tokens
                  </span>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {m.status === "running" && (
                <CyberButton variant="outline" onClick={handlePause} disabled={isMutating} className="text-sm py-2 px-4">
                  <Pause className="w-4 h-4 mr-1.5" /> Pause
                </CyberButton>
              )}
              {m.status === "paused" && (
                <CyberButton variant="primary" onClick={handleResume} disabled={isMutating} className="text-sm py-2 px-4">
              ...
              <CyberButton
                variant="danger"
                onClick={handleAbort}
                disabled={isMutating || m.status === "aborted" || m.status === "completed" || m.status === "failed"}
                className="text-sm py-2 px-4"
              >
                <Square className="w-4 h-4 mr-1.5" /> Abort
              </CyberButton>
              <button
                onClick={() => { invalidateAll(); refetchLogs(); }}
          ...
          <div className="mt-4 flex items-center gap-3">
            <span className="text-sm font-mono font-bold text-white w-10 text-right">{Math.round(m.progress || 0)}%</span>
            ...
            {m.status === "failed" && <AlertTriangle className="w-5 h-5 text-rose-400 shrink-0" />}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
          ...
          <div className="w-full lg:w-64 xl:w-72 shrink-0 border-r border-border bg-card/30 p-4 overflow-y-auto">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
              <Activity className="w-3.5 h-3.5" /> Agent Swarm ({agents.filter((a: any) => a.enabled).length})
            </h3>
            <div className="space-y-3">
              {agents.filter((a: any) => a.enabled).length === 0 ? (
                ...
                agents.filter((a: any) => a.enabled).map((agent: any, i: number) => {
                  const Icon = AGENT_ICONS[agent.agentType] || Activity;
                  const color = AGENT_COLORS[agent.agentType] || "text-white/60";
                  const recentLog = logs.filter(l => l.agentType === agent.agentType).at(-1);
                  return (
                    <GlassCard key={i} className={`p-4 border-l-2 ${m.status === "running" ? "border-l-primary/60" : "border-l-border"}`}>
                      ...
                      {recentLog && (
                        <div className={`text-xs font-mono ${color} bg-background/50 p-2 rounded border border-white/5 line-clamp-2 leading-relaxed`}>
            ...
            <div className="flex-1 overflow-hidden">
              <AnimatePresence mode="wait">
                ...
                {activeTab === "code" && (
                  ...
                  >
                    {logs.filter(l => l.agentType === "coder").length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                        <Code2 className="w-10 h-10 opacity-20 mb-3" />
                        <p className="text-sm">No code output yet</p>
                        <p className="text-xs mt-1 opacity-60">Code generated by the Coder agent will appear here</p>
                      </div>
                    ) : (
...
{logs
  ...
  .map(l => `// [${format(new Date(l.createdAt), "HH:mm:ss")}]\n// ${l.action}`)
  .join("\n\n")}
                        </code>
                      </pre>
                    )}
                  ...
                  >
                    {logs.length === 0 ? (
                      ...
                      logs.map((log, i) => {
                        const color = AGENT_COLORS[log.agentType] || "text-white/60";
                        return (
                          <div key={log.id} className={`flex gap-3 ${i % 2 === 0 ? "" : "flex-row-reverse"}`}>
                            <div className={`w-8 h-8 rounded-full bg-background border border-border flex items-center justify-center shrink-0 ${color}`}>
                              <span className="text-[10px] font-bold">{log.agentType.charAt(0).toUpperCase()}</span>
                            </div>
                            <div className={`max-w-[75%] ${i % 2 === 0 ? "" : "items-end"}`}>
                              ...
                              <p className={`text-sm text-white/80 p-3 rounded-2xl border border-white/5 leading-relaxed ${
                                i % 2 === 0 ? "bg-white/5 rounded-tl-none" : "bg-primary/10 rounded-tr-none"
                              }`}>
                                {log.action}
                              </p>
                            </div>
          ...
          <div className="w-full lg:w-72 xl:w-80 shrink-0 border-l border-border bg-card/30 flex flex-col">
              ...
              {logs.length === 0 ? (
                ...
                [...logs].reverse().map((log) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-4 h-4 rounded-full bg-background border-2 mt-0.5 shrink-0 flex items-center justify-center border-primary/40`}>
            ...
            <div className="p-4 border-t border-border space-y-2.5 shrink-0">
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Stats</h4>
              {[
                { label: "Tokens Used", value: (m.tokensUsed || 0).toLocaleString() },
                { label: "Progress", value: `${Math.round(m.progress || 0)}%` },
                { label: "Duration", value: formatElapsed(elapsed) },
                { label: "Priority", value: (m.priority || "medium"), className: "capitalize" },
              ].map(stat => (
                <div key={stat.label} className="flex justify-between text-xs">
                  <span className="text-muted-foreground">{stat.label}</span>
                  <span className={`text-white font-mono font-medium ${stat.className || ""}`}>{stat.value}</span>
                </div>
              ))}
            </div>
          </div>
