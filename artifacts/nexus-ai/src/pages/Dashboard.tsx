...
export default function Dashboard() {
  ...
  const todayVsYesterday =
    a.tasksCompletedYesterday > 0
      ? Math.round(
          ((a.tasksCompletedToday - a.tasksCompletedYesterday) /
            a.tasksCompletedYesterday) *
            100,
        )
      : 0;
  ...
  const activeMissions = missionList.filter(
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Overview of your autonomous agents and active missions.
            </p>
          </div>
            ...
            >
              <GlassCard className="p-5 group hover:bg-white/5 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div
                    className={`p-2 rounded-lg border ${s.bg} ${s.color} group-hover:scale-110 transition-transform`}
                  >
            ...
            {isLoadingMissions ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-32 bg-white/5 rounded-2xl animate-pulse border border-white/5"
                  />
                ))}
              </div>
              ...
              <GlassCard className="p-12 text-center">
                ...
                <p className="text-muted-foreground/60 text-sm mb-6">
                  Launch a new mission to get started
                </p>
                <Link href="/dashboard/new-mission">
                          ...
                          <div className="flex-1 space-y-3 w-full">
                            ...
                            <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                              <div className="flex items-center gap-1.5">
                                ...
                                {formatElapsed(
                                  mission.elapsedSeconds || 0,
                                )}{" "}
                                elapsed
                              </div>
                              <div className="flex gap-1 flex-wrap">
                                {(Array.isArray(mission.agents)
                                  ? mission.agents
                                  : []
                                )
                    ...
                    >
                      <div className="flex items-center justify-between p-4 rounded-xl bg-white/3 border border-white/5 hover:bg-white/5 transition-colors cursor-pointer group">
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          ...
                          <span>{formatRelativeDate(mission.createdAt)}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
                ...
                {isLoadingActivity ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-muted-foreground/60 text-xs mt-1">
                      Launch a mission to see live agent activity
                    </p>
                  </div>
                ) : (
                  activityList.map((log, i) => (
                    <motion.div
                      key={log.id}
